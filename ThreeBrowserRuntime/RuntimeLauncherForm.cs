...
internal sealed partial class RuntimeLauncherForm : Form
{
    ...
    private async void OnWebMessageReceived(object? sender, CoreWebView2WebMessageReceivedEventArgs e)
    {
        try
        {
            ...
            {
                case "run": await RunAsync(root.GetProperty("url").GetString() ?? "",
                    root.TryGetProperty("beautifyJavaScript", out var beautify) && beautify.ValueKind == JsonValueKind.True); break;
    ...
    private async Task RunAsync(string rawAddress, bool beautifyJavaScript = false)
    {
        if (_running) return;
        ...
        try
        {
            List<string> pullArguments = [_sitePuller, rawAddress, _destination, "--force"];
            if (beautifyJavaScript) pullArguments.Add("--beautify-js");
            var exitCode = await RunProcessAsync(_nodeExecutable, _runtimeDirectory,
                pullArguments, _operation.Token);
            if (exitCode != 0)
            {
