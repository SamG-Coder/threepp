...
export class NativeRtxRenderer {
  constructor(renderer, camera, rtx) {
    ...
    this._displayScene.name = "Native moonwater and JS HUD presentation";
  ...
  async configure(width, height, staticScene) {
    ...
    try {
      if (!this.sceneRegistered) {
        ...
        if (!registration?.queued) throw new Error("Static cove scene registration was not queued.");
        const ready = await waitForStaticScene(this.rtx);
        if (!ready.ready) {
          throw new Error(ready.feature?.reason || "The cove BLAS/TLAS did not become ready before timeout.");
        }
        this.sceneRegistered = true;
        console.log(
          `[Moonwater RTX] Static cove ready: ${staticScene.vertexCount.toLocaleString()} vertices` +
      ...
      this.failure = `Native cove setup failed: ${error?.message || error}`;
      console.warn(`[Moonwater RTX] ${this.failure}`);
  ...
  resize(width, height) {
    ...
    if (this.rasterTarget) {
      try {
        ...
        rasterReady = false;
        this.failure = `Moonwater raster resize failed: ${error?.message || error}`;
        console.error(`[Moonwater WebGPU] ${this.failure}`);
    ...
    if (this.enabled) {
      try {
        ...
        this.enabled = false;
        this.failure = `Native cove resize failed: ${error?.message || error}`;
        console.error(`[Moonwater RTX] ${this.failure}`);
        this._disposeNativeTargets();
        if (this.sceneRegistered) this.rtx?.destroyStaticScene?.();
        this.sceneRegistered = false;
      }
  ...
  _createRasterTarget(width, height) {
    ...
    target.texture.name = "Moonwater WebGPU raster composite";
  ...
  _createTargets(width, height) {
    ...
    depthTexture.name = "Moonwater RTX depth";
    ...
    const outputTarget = this._makeStorageTarget(width, height, "Moonwater native composite");
  ...
  _evaluateLighting(frameOptions, layouts) {
    ...
    try {
      const encoder = this.device.createCommandEncoder({ label: "Moonwater ray-tested celestial lighting" });
      ...
      console.warn(`[Moonwater RTX] Ray lighting disabled: ${error?.message || error}`);
      return false;
    }
  }
  ...
  _evaluateReflections(frameOptions, layouts) {
    ...
    try {
      const encoder = this.device.createCommandEncoder({ label: "Moonwater ray reflections fallback" });
      this.rtx.evaluateRayReflections({
        ...
        // The visible sky and its water response are authored by the sample's
        // Three.js/TSL material. Native RTX contributes geometry hits only;
        // a constant miss colour here would add a second, unrelated sky.
      ...
      console.warn(`[Moonwater RTX] Ray reflections disabled: ${error?.message || error}`);
      return false;
    }
  }
  ...
  render(scene, camera = this.camera, frameOptions = {}) {
    ...
    try {
      ...
      this._lastFrameNative = false;
      this.lastPath = "raster-fallback";
      this.failure = `Native Moonwater evaluation stopped: ${error?.message || error}`;
      console.error(`[Moonwater RTX] ${this.failure}`);
  ...
  renderRaster(scene, camera = this.camera) {
    try {
      ...
      this.failure = `Moonwater raster target stopped: ${error?.message || error}`;
      console.error(`[Moonwater WebGPU] ${this.failure}`);
      if (error?.stack) console.error(error.stack);
      this.renderer.setRenderTarget(null);
      this.renderer.setMRT(null);
  ...
  present(hudTexture = null, debugMode = 0) {
    ...
    try {
      ...
      this.failure = `Moonwater frame presentation stopped: ${error?.message || error}`;
      console.error(`[Moonwater WebGPU] ${this.failure}`);
      return false;
    }
  }
