...
/**
 * Offscreen native visibility/reflection boundary for the neon-mercury sample.
...
export class NativeRtxRenderer {
  constructor(
    ...
    this._displayScene.name = "Neon Mercury one-surface HDR presentation";
    ...
    this._displayQuad.name = "ACES-presented neon-mercury HDR frame";
  ...
  _createRasterTarget(width, height) {
    ...
    target.texture.name = "Neon Mercury linear raster fallback";
  ...
  _createNativeTargets(width, height) {
    ...
    depthTexture.name = "Neon Mercury RTX depth";
    ...
    const outputTarget = this._makeStorageTarget(
      width,
      height,
      "Neon Mercury native reflected HDR output",
    );
    ...
    if (!sourceColor || !normalRoughness || !specularAlbedo || !depth || !outputColor) {
      ...
      throw new Error("Three.js did not expose every neon-mercury native guide texture.");
    }
  ...
  async configure(width, height, staticScene, dynamicMesh = null) {
    ...
    try {
      ...
      this.enabled = true;
      console.log(
        `[Neon Mercury RTX] Static room ready` +
      ...
      console.warn(`[Neon Mercury RTX] ${this.failure}`);
      this._disposeNativeTargets();
      this._destroyStaticScene();
      return false;
    }
  ...
  resize(width, height) {
    ...
    try {
      ...
      this.failure = `Neon Mercury target resize failed: ${error?.message || error}`;
      console.error(`[Neon Mercury RTX] ${this.failure}`);
      this.enabled = false;
      this._disposeNativeTargets();
      return false;
    }
  ...
  _recordLighting(frameOptions) {
    ...
    try {
      const encoder = this.device.createCommandEncoder({
        label: "Neon Mercury deterministic key visibility and RTAO",
      });
  ...
  _recordReflections(frameOptions) {
    ...
    try {
      const encoder = this.device.createCommandEncoder({
        label: "Neon Mercury deterministic room reflections",
      });
      const result = this.rtx.evaluateRayReflections({
        ...
        // The live raster environment already owns miss radiance. A second
        // constant environment would double the room's authored neon energy.
  ...
  _evaluateNativeEffects(frameOptions) {
    ...
    try {
      ...
      console.warn(`[Neon Mercury RTX] Native effect submission disabled: ${failure}`);
      return 0;
    } finally {
      submitBuffers.length = 0;
    }
  ...
  renderNative(scene, camera = this.camera, frameOptions = {}) {
    ...
    try {
      ...
      this.failure = `Native neon-mercury evaluation stopped: ${error?.message || error}`;
      console.error(`[Neon Mercury RTX] ${this.failure}`);
      this.enabled = false;
  ...
  renderRaster(scene, camera = this.camera) {
    ...
    try {
      ...
      this.failure = `Neon Mercury raster staging failed: ${error?.message || error}`;
      console.error(`[Neon Mercury WebGPU] ${this.failure}`);
      this._activeTexture = null;
      this._frameReady = false;
      this.renderer.setRenderTarget(null);
      this.renderer.setMRT?.(null);
  ...
  present() {
    ...
    try {
      ...
      this.failure = `Neon Mercury presentation stopped: ${error?.message || error}`;
      console.error(`[Neon Mercury WebGPU] ${this.failure}`);
      this._frameReady = false;
      return false;
    } finally {
...
export const NeonMercuryNativeRenderer = NativeRtxRenderer;
