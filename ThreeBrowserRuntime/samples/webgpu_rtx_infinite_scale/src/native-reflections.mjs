...
export class NativeReflectionRenderer {
  constructor(renderer, camera, rtx, reflectionPipeline = null) {
    ...
    this._adaptiveStatusPollCountdown = 0;
    ...
    this._hudMaterial = this._createHudMaterial(null);
  ...
  _setHudTexture(texture) {
    if (this._hudMaterial.map === texture) return;
    // A WebGPU NodeMaterial created without a map can retain a no-texture
    // pipeline on some backends even after needsUpdate. Rebuild this one shared
    // overlay material once when the persistent HUD target becomes available.
    const previous = this._hudMaterial;
    this._hudMaterial = this._createHudMaterial(texture);
    this._hudQuad.material = this._hudMaterial;
    this._presentHudQuad.material = this._hudMaterial;
    previous.dispose();
  }
  ...
  getAdaptiveStatus() {
    const runtimeStatus = this.rtx?.getStatus?.() ?? {};
    const features = runtimeStatus.features ?? {};
  ...
  renderRaster(scene, camera = this.camera, width = innerWidth, height = innerHeight) {
    try {
      // A scale handoff intentionally uses fresh raster geometry while the
      // immutable ray atlas is non-authoritative. Never let present() reuse
      // the previous adaptive/native image during that interval.
      this.adaptiveFrameReady = false;
      this._lastFrameConstants = null;
  ...
  render(scene, camera = this.camera, frameOptions = {}) {
    ...
    try {
      ...
      if (this._adaptiveStatusPollCountdown <= 0) {
        this._syncAdaptiveFeatureRequests();
        this._adaptiveStatusPollCountdown = 11;
      } else {
        this._adaptiveStatusPollCountdown -= 1;
      }
      this.adaptiveFrameReady = false;
  ...
  present(hudTexture = null, sourceTexture = this.outputTarget?.texture ?? null) {
    ...
    try {
      if (sourceTexture === this.outputTarget?.texture &&
          this.adaptiveEnabled && this.adaptiveFrameReady &&
