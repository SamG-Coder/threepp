...
const DEFAULT_TRIANGLE_BUDGET = 600_000;
...
function appendRadiance(stream, mesh, geometry, material, a, b, c, instance) {
  ...
  const ambient = 0.008 + THREE.MathUtils.clamp(finite(material?.metalness, 0), 0, 1) * 0.004;
  stream.push(
...
/**
 * Pack at most eight static point/spot lights for the public reflections-v2
 * scene ABI. Directional moon light remains a per-frame lighting parameter.
 */
export function packStaticRtxLights(lights = []) {
  ...
  for (const light of lights ?? []) {
    ...
    const range = Math.max(0.25, finite(light.distance, 0) || 1_200);
...
/**
 * Cooperatively snapshot opaque, non-deforming Three.js geometry into the
 * Runtime's one static ray-query scene. Every accepted triangle is expanded
 * to three world-space vertices so material filtering and instancing remain
 * exactly aligned with the per-triangle radiance/surface streams.
 *
 * Objects or materials carrying `userData.rtxIgnore`, including the moving
 * flood surface, are excluded together with hidden, transparent and animated
 * geometry. The result is bounded even when an instanced source exceeds the
 * requested budget.
 */
export async function collectStaticRtxScene(objects, options = {}) {
    ...
    while (stack.length > 0) {
      ...
      for (let instance = 0; instance < instanceCount; ++instance) {
        ...
        for (let cursor = range.start; cursor + 2 < range.end; cursor += 3) {
          ...
          const a = vertexIndex(position, sourceIndex, cursor);
          const b = vertexIndex(position, sourceIndex, cursor + 1);
          const c = vertexIndex(position, sourceIndex, cursor + 2);
  ...
  if (triangleCount === 0) {
    throw new Error("Flash Flood Gorge contains no opaque static RTX triangles.");
  }
...
export const collectStaticGorgeScene = collectStaticRtxScene;
