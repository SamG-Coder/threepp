...
const BLACK_ENVIRONMENT = Object.freeze([0, 0, 0]);
const DEFAULT_MOON_DIRECTION = Object.freeze([-0.34, 0.82, -0.46]);
...
function normalizeDirectionInto(target, value, fallback = DEFAULT_MOON_DIRECTION) {
  let x = fallback[0];
  let y = fallback[1];
  let z = fallback[2];
  if (value?.isVector3) {
...
/**
 * Offscreen native visibility/reflection boundary for the flash-flood sample.
 ...
 */
export class NativeRtxWaterRenderer {
  constructor(
    renderer,
    camera,
    ...
    if (!renderer || !camera) {
      throw new TypeError("NativeRtxWaterRenderer requires a renderer and camera.");
    }
    ...
    this._displayScene.name = "Flash-flood one-surface HDR presentation";
    ...
    this._displayQuad.name = "ACES-presented flash-flood HDR frame";
    this._displayQuad.frustumCulled = false;
    this._displayScene.add(this._displayQuad);

    // Most transparent reflective materials use an opaque MRT guide. The
    // flood surface explicitly preserves alpha so shallow bed detail remains
    // visible; its coverage-weighted guide is intentionally blended instead.
  ...
  _destroyStaticScene() {
    ...
    try {
      ...
      console.warn(`[Flash Flood RTX] Static-scene release failed: ${error?.message || error}`);
    }
    this.sceneRegistered = false;
  }
  ...
  _createRasterTarget(width, height) {
    ...
    target.texture.name = "Flash-flood linear raster fallback";
  ...
  _createNativeTargets(width, height) {
    ...
    depthTexture.name = "Flash-flood RTX depth";
    ...
    const outputTarget = this._makeStorageTarget(
      width,
      height,
      "Flash-flood native reflected HDR output",
    );
    this.renderer.initRenderTarget(sceneTarget);
    this.renderer.initRenderTarget(outputTarget);

    const sourceColor = this.renderer.backend?.get?.(sceneTarget.textures[0])?.texture ?? null;
    ...
    if (!sourceColor || !normalRoughness || !specularAlbedo || !depth || !outputColor) {
      ...
      throw new Error("Three.js did not expose every flash-flood native guide texture.");
    }
  ...
  async configure(width, height, staticScene) {
    ...
    try {
      ...
      if (!registration || registration.queued === false) {
        throw new Error(registration?.reason || "Static gorge scene registration was rejected.");
      }
      this.sceneRegistered = true;
      const ready = await waitForStaticScene(this.rtx, this.options.timeoutMs);
      if (!ready.ready) {
        throw new Error(
          ready.feature?.reason || "The gorge BLAS/TLAS did not become ready before timeout.",
        );
      }
      ...
      this.enabled = true;
      console.log(
        `[Flash Flood RTX] Static gorge ready` +
      ...
      this.failure = `Native gorge setup failed: ${error?.message || error}`;
      console.warn(`[Flash Flood RTX] ${this.failure}`);
      this._disposeNativeTargets();
      this._destroyStaticScene();
      return false;
    }
  ...
  resize(width, height) {
    ...
    try {
      ...
      this.failure = `Flash-flood target resize failed: ${error?.message || error}`;
      console.error(`[Flash Flood RTX] ${this.failure}`);
      this.enabled = false;
      this._disposeNativeTargets();
      return false;
    }
  ...
  _recordLighting(frameOptions) {
    ...
    try {
      const encoder = this.device.createCommandEncoder({
        label: "Flash-flood deterministic moon visibility and RTAO",
      });
      ...
      const result = this.rtx.evaluateRayLighting({
        ...
        // sequence prevents the gorge walls and water highlights from flashing.
        frameIndex: 0,
      ...
      if (result?.queued === false) {
        throw new Error(result.reason || "Native moon lighting was rejected.");
      }
      return encoder.finish();
    } catch (error) {
      this.lightingEnabled = false;
      this.lightingFailure = String(error?.message || error);
      console.warn(`[Flash Flood RTX] Ray lighting disabled: ${this.lightingFailure}`);
      return null;
    }
  }
  ...
  _recordReflections(frameOptions) {
    ...
    try {
      const encoder = this.device.createCommandEncoder({
        label: "Flash-flood deterministic gorge and bridge reflections",
      });
      const result = this.rtx.evaluateRayReflections({
        ...
        // The live TSL sky and water already own the miss radiance. A second
        // constant environment here would double the moonlit water energy.
        environmentColor: BLACK_ENVIRONMENT,
      ...
      if (result?.queued === false) {
        throw new Error(result.reason || "Native gorge reflections were rejected.");
      }
      return encoder.finish();
    } catch (error) {
      this.reflectionsEnabled = false;
      this.reflectionFailure = String(error?.message || error);
      console.warn(`[Flash Flood RTX] Ray reflections disabled: ${this.reflectionFailure}`);
      return null;
    }
  }
  ...
  _evaluateNativeEffects(frameOptions) {
    ...
    try {
      // evaluateRayReflections currently requires a dedicated empty logical
      // encoder, so combining both effects into one command buffer would be an
      // API violation. One queue submission of two command buffers preserves
      // lighting -> reflection order and is the maximum safe coalescing.
      ...
      console.warn(`[Flash Flood RTX] Native effect submission disabled: ${failure}`);
      return 0;
    } finally {
      submitBuffers.length = 0;
    }
  ...
  renderNative(scene, camera = this.camera, frameOptions = {}) {
    ...
    try {
      ...
      const suppressRasterLights = this.lightingEnabled
        ? frameOptions.suppressRasterLights ?? EMPTY_ARRAY
        : EMPTY_ARRAY;
      ...
      this.failure = `Native flash-flood evaluation stopped: ${error?.message || error}`;
      console.error(`[Flash Flood RTX] ${this.failure}`);
      this.enabled = false;
  ...
  renderRaster(scene, camera = this.camera) {
    ...
    try {
      ...
      this.failure = `Flash-flood raster staging failed: ${error?.message || error}`;
      console.error(`[Flash Flood WebGPU] ${this.failure}`);
      this._activeTexture = null;
      this._frameReady = false;
      this.renderer.setRenderTarget(null);
      this.renderer.setMRT?.(null);
  ...
  present() {
    ...
    try {
      ...
      this.failure = `Flash-flood presentation stopped: ${error?.message || error}`;
      console.error(`[Flash Flood WebGPU] ${this.failure}`);
      this._frameReady = false;
      return false;
    } finally {
...
export const FlashFloodNativeRenderer = NativeRtxWaterRenderer;
