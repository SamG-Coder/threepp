...
async function pathsFor(slot) {
  ...
  const directory = path.join(local, "ThreeBrowser", "GtaNeonCity", "saves");
...
export function createSaveService() {
  return {
    async save(slot, snapshot) {
      const paths = await pathsFor(String(slot || "quicksave"));
      const envelope = { version: SAVE_VERSION, savedAt: new Date().toISOString(), snapshot };
    ...
    async load(slot) {
      ...
      try {
        ...
        if (envelope?.version !== SAVE_VERSION || !envelope.snapshot) throw new Error("Save file version is not supported");
