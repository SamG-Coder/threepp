...
import { BANK_LIGHTING_GLSL } from "./bank-lighting.mjs";
...
export class NativeRtxRenderer {
  constructor(renderer, camera, rtx) {
    ...
    this._displayScene.name = "Native Secret River presentation";
  ...
  async configure(width, height, staticScene) {
    ...
    try {
      if (!this.sceneRegistered) {
        ...
        if (!registration?.queued) throw new Error("Static riverbank scene registration was not queued.");
        const ready = await waitForStaticScene(this.rtx);
        if (!ready.ready) {
          throw new Error(ready.feature?.reason || "The riverbank BLAS/TLAS did not become ready before timeout.");
        }
        this.sceneRegistered = true;
        console.log(
          `[Secret River RTX] Static riverbank ready: ${staticScene.vertexCount.toLocaleString()} vertices` +
          ` · ${staticScene.triangleCount.toLocaleString()} triangles` +
          ` · rayLighting=${this.lightingEnabled}` +
          ` · rayReflections=${this.reflectionsEnabled}`,
        );
      }
      if (!this._lightingPipeline && this.lightingEnabled &&
          typeof this.rtx.compileRayQueryPipeline === "function") {
        try {
          this._lightingPipeline = await this.rtx.compileRayQueryPipeline({
            profile: "lighting-v1",
            source: BANK_LIGHTING_GLSL,
            language: "glsl",
            stage: "compute",
            entryPoint: "main",
            label: "Secret River 2.5D bank lighting",
          });
      ...
      this.failure = `Native riverbank setup failed: ${error?.message || error}`;
      console.warn(`[Secret River RTX] ${this.failure}`);
  ...
  resize(width, height) {
    ...
    if (this.rasterTarget) {
      try {
        ...
        rasterReady = false;
        this.failure = `Secret River raster resize failed: ${error?.message || error}`;
        console.error(`[Secret River WebGPU] ${this.failure}`);
    ...
    if (this.enabled) {
      try {
        ...
        this.enabled = false;
        this.failure = `Native riverbank resize failed: ${error?.message || error}`;
        console.error(`[Secret River RTX] ${this.failure}`);
        this._disposeNativeTargets();
        if (this.sceneRegistered) this.rtx?.destroyStaticScene?.();
        this.sceneRegistered = false;
      }
  ...
  _createRasterTarget(width, height) {
    ...
    target.texture.name = "Secret River WebGPU raster composite";
  ...
  _createTargets(width, height) {
    ...
    depthTexture.name = "Secret River RTX depth";
    ...
    const outputTarget = this._makeStorageTarget(width, height, "Secret River native composite");
  ...
  _evaluateLighting(frameOptions, layouts) {
    if (frameOptions.skipLighting) return false;
    if (!this.lightingEnabled || typeof this.rtx.evaluateRayLighting !== "function") return false;
    // The generic lighting-v1 shader self-shadows 2.5D cards to black.
    if (!this._lightingPipeline) return false;
    try {
      const encoder = this.device.createCommandEncoder({ label: "Secret River ray-tested celestial lighting" });
      this.rtx.evaluateRayLighting({
        pipeline: this._lightingPipeline,
        ...
        shadowStrength: Number(frameOptions.shadowStrength ?? 0.48),
        aoStrength: Number(frameOptions.aoStrength ?? 0.16),
        aoRadius: 1.4,
        depthInverted: false,
      });
      ...
      this.lightingEnabled = false;
      console.warn(`[Secret River RTX] Ray lighting disabled: ${error?.message || error}`);
      return false;
    }
  }

  _evaluateReflections(frameOptions, layouts) {
    if (frameOptions.skipReflections) return false;
    if (!this.reflectionsEnabled || typeof this.rtx.evaluateRayReflections !== "function") return false;
    try {
      const encoder = this.device.createCommandEncoder({ label: "Secret River ray reflections fallback" });
      this.rtx.evaluateRayReflections({
        ...
        inverseViewProjection: this._inverseViewProjection.toArray(),
        cameraPosition: this._cameraPosition,
        reflectionStrength: Number(frameOptions.reflectionStrength ?? 0.96),
        maxDistance: Number(frameOptions.maxDistance ?? 96),
        rayBias: Number(frameOptions.rayBias ?? 0.018),
        roughnessCutoff: 0.88,
        environmentColor: vectorArray(frameOptions.environmentColor, [0.46, 0.58, 0.70]),
        environmentIntensity: Number(frameOptions.environmentIntensity ?? 1.15),
      ...
      console.warn(`[Secret River RTX] Ray reflections disabled: ${error?.message || error}`);
      return false;
    }
  }

  _renderLinearScene(scene, camera, target, mrtLayout = null, options = {}) {
    ...
    try {
      ...
      this.renderer.autoClear = options.clear !== false;
      if (options.clear !== false) this.renderer.clear(true, true, true);
  ...
  render(scene, camera = this.camera, frameOptions = {}) {
    ...
    try {
      ...
      this._lastFrameNative = false;
      this.lastPath = "raster-fallback";
      this.failure = `Native Secret River evaluation stopped: ${error?.message || error}`;
      console.error(`[Secret River RTX] ${this.failure}`);
  ...
  renderRaster(scene, camera = this.camera) {
    try {
      ...
      this.failure = `Secret River raster target stopped: ${error?.message || error}`;
      console.error(`[Secret River WebGPU] ${this.failure}`);
      if (error?.stack) console.error(error.stack);
      this.renderer.setRenderTarget(null);
      this.renderer.setMRT(null);
  ...
  present(hudTexture = null, debugMode = 0, overlayOpacity = 0) {
    ...
    try {
      ...
      this.failure = `Secret River frame presentation stopped: ${error?.message || error}`;
      console.error(`[Secret River WebGPU] ${this.failure}`);
      return false;
    }
  }
