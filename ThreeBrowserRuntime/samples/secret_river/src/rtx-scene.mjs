...
export function collectStaticRiverScene(roots, lights = []) {
  const positions = [];
  const indices = [];
  const radiance = [];
  const surfaces = [];
  ...
  if (positions.length === 0 || indices.length === 0) {
    throw new Error("Secret River contains no static RTX triangles.");
  }
  const triangleCount = indices.length / 3;
  if (indices.length % 3 !== 0 || radiance.length !== triangleCount * 4 || surfaces.length !== triangleCount * 4) {
    throw new Error("Secret River RTX geometry and material streams are misaligned.");
  }
  return {
